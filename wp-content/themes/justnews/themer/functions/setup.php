...
function wpcom_admin_setup() {
    ...
    new WPCOM_Meta();
    if (!wp_next_scheduled ( 'wpcom_sessions_clear' )) {
        wp_schedule_event(time(), 'hourly', 'wpcom_sessions_clear');
    }
}
...
add_action('wp_enqueue_scripts', 'wpcom_scripts', 1);
/* 静态资源结束 */
...
add_action( 'transition_post_status', 'wpcom_xzh_pre_submit', 10, 3 );
function wpcom_xzh_pre_submit( $new_status, $old_status, $post ){
    if( $new_status!='publish' || $new_status==$old_status || $post->post_type!='post' ) return false;
    if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) return false;
    //if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) return false;
    global $options;
    if(isset($post->ID) && isset($options['xzh-submit']) && $options['xzh-submit']) {
        global $xzh_pre_submit;
        $xzh_pre_submit = $post->ID;
    }
}

add_action( 'wp_insert_post', 'wpcom_xzh_submit', 50, 2 );
function wpcom_xzh_submit($post_ID, $post){
    global $xzh_pre_submit, $options;
    if(isset($xzh_pre_submit) && $post->post_status=='publish' && $xzh_pre_submit==$post_ID){
