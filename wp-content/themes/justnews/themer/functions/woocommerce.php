...
function wpcom_billing_email_filter($email){
    if(preg_match('/@email\.empty$/i', $email) || preg_match('/@weixin\.qq$/i', $email) || preg_match('/@(weapp|swan|alipay|tt|qq)\.app$/i', $email)) { // 未设置邮箱
        $email = '';
    }
    return $email;
}
...
function wpcom_woo_mail_callback_params($params, $that){
    if(preg_match('/@email\.empty$/i', $params[0]) || preg_match('/@weixin\.qq$/i', $params[0]) || preg_match('/@(weapp|swan|alipay|tt|qq)\.app$/i', $params[0])) { // 未设置邮箱
