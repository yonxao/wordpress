...
if( !function_exists( 'wpcom_account_form_bind' ) ){
    function wpcom_account_form_bind(){
        ...
        if(isset($_POST['by']) && $_POST['by']){
            ...
            }else if($by == 'email' && (!$user->user_email || preg_match('/@email\.empty$/i', $user->user_email) || preg_match('/@weixin\.qq$/i', $user->user_email) || preg_match('/@(weapp|swan|alipay|tt|qq)\.app$/i', $user->user_email)) ){
    ...
    function wpcom_ajax_register(){
        ...
        if ($res['result'] == 1) {
            ...
            if ( is_wp_error( $user_id ) ){
                ...
                if( !is_wpcom_enable_phone() && isset($options['member_reg_active']) && $options['member_reg_active']!='0' ){
                    // 邮箱注册用户需要验证
        ...
        if( isset( $_POST['user_login'] ) && is_string( $_POST['user_login'] ) ){
            ...
            if( $user ) {
                if(!$is_mobile_phone) { // 非手机找回，则发送邮件
                    ...
                    if(!$user->user_email || preg_match('/@email\.empty$/i', $user->user_email) || preg_match('/@weixin\.qq$/i', $user->user_email) || preg_match('/@(weapp|swan|alipay|tt|qq)\.app$/i', $user->user_email)){ // 未设置邮箱
                        if($phone){// 使用手机找回
                            $is_mobile_phone = 1;
...
function wpcom_send_sms_code(){
    ...
    if ($res['result'] == 1) {
        if(is_email($_POST[$target])){
            ...
            $send = wpcom_qcloud_sms_code($_POST[$target]);
            if($send->result!==0){ // 发送失败
                $res['result'] = 0;
                $res['error'] = $send->errmsg;
            }
        }
        if($res['result'] == 1 && isset($_POST['ticket'])){
            $ticket = $_POST['ticket'];
            $randstr = $_POST['randstr'];
            WPCOM_Session::set('last_ticket', $ticket . '+' . $randstr);
        }
    }
