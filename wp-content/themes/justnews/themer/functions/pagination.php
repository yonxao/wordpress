...
function wpcom_pagination( $range = 9, $args = array() ) {
    ...
    if ( ($args && $args['numpages'] > 1) || ( isset($multipage) && $multipage && is_single() ) ) {
        ...
        echo '<form class="pagination-go" method="get"><input class="pgo-input" type="text" name="paged" placeholder="'._x('Paged', '页码', 'wpcom').'" /><button class="pgo-btn" type="submit"><i class="fa fa-arrow-circle-right"></i></button></form>';
        echo '</div>';
    }
}
