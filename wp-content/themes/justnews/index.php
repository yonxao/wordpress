                ...
                <div class="sec-panel topic-recommend">
                    <?php if(isset($options['special_home_title']) && $options['special_home_title']){ ?>
                        <div class="sec-panel-head">
                            <h2><span><?php echo $options['special_home_title'];?></span> <small><?php echo $options['special_home_desc'];?></small> <?php if(isset($options['special_home_url']) && $options['special_home_url']){ ?><a href="<?php echo esc_url($options['special_home_url']);?>" target="_blank" class="more"><?php $more_special = isset($options['more_special']) && $options['more_special'] ? $options['more_special'] : __('All Topics', 'wpcom'); echo $more_special;?></a><?php } ?></h2>
                        </div>
                    <?php } ?>
        ...
        <div class="sec-panel">
            <?php if($partners && $partners[0]){
                if(isset($options['partner_title']) && $options['partner_title']){
                    ?>
                    <div class="sec-panel-head">
                        <h2><span><?php echo $options['partner_title'];?></span> <small><?php echo $options['partner_desc'];?></small> <a href="<?php echo esc_url($options['partner_more_url']);?>" target="_blank" class="more"><?php echo $options['partner_more_title'];?></a></h2>
                    </div>
                <?php } ?>
                <div class="sec-panel-body">
                    <ul class="list list-partner">
                        ...
                        foreach($partners as $x =>$pt){
                            ...
                            <li style="width:<?php echo $width;?>%">
                                <?php if($url){ ?><a target="_blank" title="<?php echo esc_attr($alt);?>" href="<?php echo esc_url($url);?>" rel="nofollow"><?php } ?><?php echo wpcom_lazyimg($pt, $alt);?><?php if($url){ ?></a><?php } ?>
                            </li>
                        <?php } ?>
                    </ul>
                </div>
            ...
            if($bookmarks){
                if(isset($options['link_title']) && $options['link_title']){
                    ?>
                    <div class="sec-panel-head">
                        <h2><span><?php echo $options['link_title'];?></span> <small><?php echo $options['link_desc'];?></small> <a href="<?php echo esc_url($options['link_more_url']);?>" target="_blank" class="more"><?php echo $options['link_more_title'];?></a></h2>
                    </div>
                <?php } ?>
