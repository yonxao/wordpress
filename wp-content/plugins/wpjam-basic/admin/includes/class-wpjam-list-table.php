...
class WPJAM_List_Table extends WP_List_Table {
	...
	protected function get_row_actions($actions, $id){
		$row_actions	= [];
		...
		foreach ($actions as $action_key => $action){
			...
			if(isset($this->_args['post_type'])){
				if(isset($action['post_status'])){
					...
					if(!in_array(get_post($id)->post_status, $post_statuses)){
						continue;
					}
				}
			}elseif(isset($this->_args['taxonomy'])){
				if(isset($action['parent'])){
					if(get_term($id, $this->_args['taxonomy'])->parent != $action['parent']){
						continue;
					}
				}
			}
	...
	protected function parse_item($raw_item){
		...
		if($primary_key && $actions){
			...
			$row_actions	= $this->get_row_actions($actions, $item_id);
	...
	public function list_action($list_action='', $id=0, $data=null){
		...
		if($bulk){
			foreach($ids as $_id){
				$result	= apply_filters($list_action_hook, $result, $list_action, $_id, $data, $page_data_type);
