...
add_filter('wpjam_basic_setting', function(){
	...
	if($wp_rewrite->use_verbose_page_rules){
		...
		$enhance_fields['no_category_base_set']['value']	= '你的固定链接设置使得去掉分类目录链接中的 category 或者自定义分类的 %taxonomy%，请先修改固定链接。';
	}
...
add_action('admin_head', function(){
	...
	jQuery(function ($){
		...
		$('#no_category_base').on('change', function(){
			if($(this).is(':checked')){
				$('#div_no_category_base_for').show();
			}else{
				$('#div_no_category_base_for').hide();
			}
		});

		$('#no_category_base').change();
	});
	</script>
	<?php
});
