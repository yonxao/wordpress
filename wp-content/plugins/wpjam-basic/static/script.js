jQuery(function($){
	...
	$.extend({
			...
			$.post(ajaxurl, args, function(data, status){
				...
				if(response.errcode != 0){
					...
					if(list_action_type == 'list'){
						...
						if(wpjam_page_setting.plugin_page && !wpjam_page_setting.bulk){
							var replace_url	= $.wpjam_admin_url()+'&action='+args.list_action;
						...
						if(wpjam_page_setting.plugin_page && response_type != 'form' && response.next_action){
							var replace_url	= $.wpjam_admin_url()+'&action='+response.next_action;
							if(response.next_action != 'add' && response.id){
								replace_url	+= '&id='+response.id;
							}
		...
		wpjam_list_table_query_items: function(pushState){
			...
			if(pushState){
				var push_url = $.wpjam_admin_url(true)+'&'+$.param(wpjam_page_setting.params);
		...
		wpjam_tab_nav: function(current_tab){
			...
			if(current_tab == ''){ //设置第一个为当前 tab显示
				...
				if(wpjam_page_setting.current_option && !wpjam_page_setting.current_tab){
			...
			if(wpjam_page_setting.current_option && !wpjam_page_setting.current_tab){
				if(current_tab != wpjam_page_setting.params.option_tab){
					wpjam_page_setting.params.option_tab	= current_tab;
					window.history.replaceState(null, null, $.wpjam_admin_url(true)+'&option_tab='+current_tab);
					$('input[name="_wp_http_referer"]').val($.wpjam_admin_url(true)+'&option_tab='+current_tab);
				}
			}	
		},

		wpjam_admin_url(base){
			var params	= {};

			base	= base || false;
			if(base){
				if($('#wpjam_query_data').length > 0){
					params	= JSON.parse($('#wpjam_query_data').val());
				}
			}
			...
			$.each(window.location.search.substring(1).split('&'), function(){
				...
				if(base){
					...
					if($.inArray(query[0], ['action', 'id', 'data']) == -1){
						params[query[0]]	= query[1];
					}
				}
			});
	...
	$('body').on('click', '.list-table-action', function(){
		...
		// 	wpjam_page_setting.action_flows = wpjam_page_setting.action_flows || new Array();
		// 	if(wpjam_page_setting.action_flows.length){
		// 		var last = wpjam_page_setting.action_flows.pop();
		// 		if(last == list_action){
		// 			args.prev_action	= wpjam_page_setting.action_flows.pop();
		// 		}else{
		// 			wpjam_page_setting.action_flows.push(last);
		// 		}
		// 	}
		// }
